...
function modeChips(behaviorId) {
  ...
  return '<div class="mode-block panel"><h2 class="panel-title">出现方式</h2><div class="mode-row" role="group" aria-label="出现方式">' + chips + '</div><p class="mode-why" id="mode-face-hint">换堆叠或弹幕，外观不会另起一套。</p><p class="mode-why" id="popup-why">突脸还在开发中，暂时不能选择。现在只能用堆叠或弹幕。</p></div>';
}
...
function stackSection(props, behaviorId) {
  ...
  return '<details class="fold panel" id="stack-section"' + (behaviorId === 'ticker' ? ' hidden' : ' open') + ' aria-label="堆叠怎么出现">'
    ...
    + '<div class="field"><span class="label">悬停</span>' + hoverHighlightChip('prop-hover-highlight', props.hoverHighlight === 'on', false) + '</div>'
    + '</div>'
    + '<p class="field-note">停留 1–120 秒。悬停加亮默认关，打开后鼠标放上去卡片会亮一点。</p>'
    + '</div></div>'
    + '<details class="more"><summary>边距与卡片间距</summary><p class="group-hint">只改停靠那两面。另外两面灰色，不挤空间。</p><div class="fields" style="margin-top:12px">'
...
function tickerSection(ticker, behaviorId, hoverHighlight) {
  ...
  return '<details class="fold panel ticker-section" id="ticker-section"' + (behaviorId === 'ticker' ? ' open' : ' hidden') + ' aria-label="弹幕怎么流">'
    + '<summary>弹幕怎么流 <small id="ticker-direction-copy">' + (flyRight ? '从左往右，看过即走' : '从右往左，看过即走') + '</small></summary>'
    + '<div class="fold-body">'
    + '<div class="chip-row" role="group" aria-label="弹幕方向">'
    ...
    + '</div>'
    + '<select id="ticker-direction" class="mode-contract-select" aria-label="弹幕方向" tabindex="-1" aria-hidden="true">'
    + '<option value="left"' + (!flyRight ? ' selected' : '') + '>右 → 左</option>'
    + '<option value="right"' + (flyRight ? ' selected' : '') + '>左 → 右</option></select>'
    ...
    + '</div>'
    + '<p class="field-note">开着不挡点击：弹幕从鼠标上穿过，点不到、也拖不走。</p>'
...
function appearanceSection(props, appearance, skin, assetOptions, parts = {}, glossary = {}, fonts = [], studioAgents = []) {
  ...
  const visibleCloseIconColor = typeof selectedPaint.closeIconColor === 'string' && /^#[0-9a-fA-F]{6}$/.test(selectedPaint.closeIconColor)
    ...
    : '#d1e0e0';
  return '<details class="fold panel is-quiet" id="appearance-section" open aria-label="卡片外观">'
    + '<summary>卡片外观 <small>根或一件零件</small></summary><div class="fold-body"><p class="group-hint">没选中孩子就是在编根。点标题、正文、关闭、图标或助手名，只出那一件的位置、文字、背景和描边。</p>'
    ...
    + '<div class="field span-2" id="bg-asset-field"><span class="label">底图</span><select id="skin-bg-asset" aria-label="底图"><option value="">没有图 · 纯色</option>' + assetOptions + '</select><div class="chip-row"><button type="button" id="bg-asset-import" class="chip">导入</button><button type="button" id="bg-asset-library" class="chip">素材库</button><button type="button" id="bg-adjust-open" class="chip">调整底图</button></div><p class="field-note">没选图就是纯色。选了图再调整大小和位置。</p></div>'
...
const CSS_PART_EXTRAS = '.paint-group{display:grid;gap:8px;padding:14px 0 4px;border-top:1px solid var(--line)}.paint-kicker{margin:0;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--muted)}.paint-group .fields{max-width:none}.paint-group .field-note{margin:0}'
  + '.stack-layout{display:grid;grid-template-columns:168px 168px minmax(160px,.9fr) minmax(240px,1.2fr);gap:16px 24px;align-items:start}'
