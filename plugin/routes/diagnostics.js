...
(function () {
  ...
  function render(status) {
    ...
    $("scene-behavior-list").innerHTML = channels.length ? channels.map(function (channel) { var channelCards = (channel.cardOrder || []).map(function (id) { return cardMap[id]; }).filter(Boolean); return '<article class="diagnostic-item"><div class="diagnostic-head"><strong class="diagnostic-code">通道：' + text(channel.channelId) + '</strong><span class="diagnostic-time">profile：' + text(channel.profileId) + '</span></div><div class="diagnostic-meta"><span>卡片顺序：' + text((channel.cardOrder || []).join(' → '), '空') + '</span></div>' + (channelCards.length ? '<div class="diagnostic-meta">' + channelCards.map(function (card) { return '<span>' + text(card.id) + ' · ' + text(card.eventId, 'unknown event') + ' · ' + text(card.behaviorChannelId, 'no channel') + ' · (' + text(card.x, '?') + ',' + text(card.y, '?') + ')</span>'; }).join('') + '</div>' : '') + '</article>'; }).join("") : (cards.length ? cards.map(function (card) { return '<article class="diagnostic-item"><div class="diagnostic-head"><strong class="diagnostic-code">卡片：' + text(card.id) + '</strong><span class="diagnostic-time">(' + text(card.x, '?') + ',' + text(card.y, '?') + ')</span></div><div class="diagnostic-meta"><span>事件：' + text(card.eventId, 'unknown event') + '</span><span>行为：' + text(card.behaviorProfileId, 'unknown') + '</span><span>通道：' + text(card.behaviorChannelId, 'unknown') + '</span></div></article>'; }).join("") : '<div class="empty">当前没有可见 Scene 卡片</div>');
