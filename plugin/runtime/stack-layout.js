...
function placeColumn(column, { workArea, margin, spacing, anchor, grow, wrapOffset }) {
  ...
  if (!isHorizontal(grow)) {
    ...
    for (const card of [...column.items].reverse()) {
      const x = rightOn
...
export function createStackLayout({ anchor = 'bottom-right', spacing = 12, margin = 20, grow, wrap } = {}) {
  ...
  return ({ workArea, cards = [] } = {}) => {
    ...
    if (resolvedWrap === 'snake') {
      const runs = binRunsOldestFirst(normalized, primaryWork, spacing, horizontal);
      let wrapUsed = 0;
    ...
    const columns = binColumns(normalized, primaryWork, spacing, horizontal);
    ...
    for (const column of columns) {
      placed.push(...placeColumn(column, { workArea, margin, spacing, anchor, grow: resolvedGrow, wrapOffset }));
