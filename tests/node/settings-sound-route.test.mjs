...
test('sound settings page exposes the simplified configuration workbench and audio library', () => {
  ...
  assert.match(html, /asset-delete-cancel/);
  assert.match(html, /beginAssetDeleteConfirm/);
  assert.match(html, /resetAssetDeleteConfirm/);
  assert.match(html, /删除后不可恢复|确认后会一并解除/);
  assert.match(html, /音频包只进出这份列表里的声音/);
  assert.match(html, /配置包只进出事件绑定和音量/);
  const comboImport = html.indexOf('导入配置包（.nhcombo）');
  const comboExport = html.indexOf('导出配置包（.nhcombo）');
  const soundImport = html.indexOf('导入音频包（.nhsound）');
  const soundExport = html.indexOf('导出音频包（.nhsound）');
  assert.ok(comboImport >= 0 && comboImport < comboExport, '配置排应先导入后导出');
  assert.ok(soundImport >= 0 && soundImport < soundExport, '音频排应先导入后导出');
  ...
  assert.doesNotMatch(html, /data-delete-warning/);
